+++
 title = 'Arc Lengths of Parametric Equations'
[params]
	author = 'Carson West'
+++
...
Arc length is a fundamental concept in calculus that allows us to measure the distance along a curve. For parametric equations, which describe a curve using a parameter (often  $ t $ ), the method for finding arc length is particularly elegant and extends concepts from [Calculus and Vector-Valued Functions](./../calculus-and-vector-valued-functions/).
...
| :-------- | :---------- | :-------- |
|  $ \frac{dx}{dt} $  | Derivative of  $ x $  with respect to  $ t $  | Represents the instantaneous rate of change of the  $ x $ -coordinate. Crucial for [Differentiating Parametric Equations](./../differentiating-parametric-equations/). |
|  $ \frac{dy}{dt} $  | Derivative of  $ y $  with respect to  $ t $  | Represents the instantaneous rate of change of the  $ y $ -coordinate. Also from [Differentiating Parametric Equations](./../differentiating-parametric-equations/). |
...
## 3. Derivation Intuition [Derivation of Arc Length Formula](./../derivation-of-arc-length-formula/)
