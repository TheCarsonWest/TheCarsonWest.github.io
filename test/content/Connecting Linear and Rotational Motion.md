...
The tangential velocity vector is always perpendicular to the radial vector pointing from the axis of rotation. If an object is not just rotating but also moving linearly (like a wheel rolling on the ground), we use these relations to describe the motion of the center of mass. See [Rolling](./../rolling/) for a deeper dive into this specific case.
...
1.  **Tangential Acceleration ( $ a_t $ ):** This component is responsible for changing the *speed* of the object. It is defined as:
     $$ a_t=r\alpha $$  2.  **Radial (Centripetal) Acceleration ( $ a_r $  or  $ a_c $ ):** This component is responsible for changing the *direction* of the velocity vector, keeping the object in a circular path. It is defined as:
