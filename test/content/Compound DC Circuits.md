...
*   **[Kirchhoff’s Junction Rule](./../kirchhoff’s-junction-rule/):** Based on the conservation of charge, the sum of currents entering a junction must equal the sum of currents leaving it.
     $$ \sum I_{in} = \sum I_{out} $$  
*   **[Kirchhoff’s Loop Rule](./../kirchhoff’s-loop-rule/):** Based on the conservation of energy, the algebraic sum of potential differences (voltage drops and gains) around any closed loop must be zero.
     $$ \sum V = 0 $$  
...
Remember that while  $ R_{eq} $  simplifies the math, the individual components still obey their specific physical properties, often discussed in [Resistance, Resistivity, and Ohm's Law](./../resistance,-resistivity,-and-ohm's-law/) and [Electric Power](./../electric-power/).
