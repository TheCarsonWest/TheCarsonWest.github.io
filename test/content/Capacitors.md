...
[Electric Potential](./../electric-potential/) explains how the separation distance  $ d $  relates to the electric field created between these plates.
...
In parallel, the potential difference  $ V $  is the same across each capacitor, but the charge  $ Q $  is divided.
 $$ C_{eq}=C_1+C_2+... $$  
Understanding how these combine is crucial when analyzing [Simple Circuits](./../simple-circuits/) or complex [Resistor-Capacitor (RC) Circuits](./../resistor-capacitor-(rc)-circuits/).