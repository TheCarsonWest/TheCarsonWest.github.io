...
For components that obey [Resistance, Resistivity, and Ohm's Law](./../resistance,-resistivity,-and-ohm's-law/), we can substitute Ohm's Law ( $ V = IR $ ) into the power equation to derive alternative forms. These are particularly useful when you only know two of the three variables ( $ I, V, R $ ).
...
Power is fundamentally linked to the energy conservation principles found in [Kirchhoff’s Loop Rule](./../kirchhoff’s-loop-rule/). In any closed loop, the total power supplied by the battery (or EMF source) must equal the total power dissipated by all components (resistors, etc.) in that circuit:
