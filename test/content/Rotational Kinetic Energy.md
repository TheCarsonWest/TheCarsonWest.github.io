...
Since the term  $ \sum m_i r_i^2 $  is defined as the [Rotational Inertia](./../rotational-inertia/) ( $ I $ ), we arrive at the fundamental equation for rotational kinetic energy:
...
| :--- | :--- | :--- |
| Inertial Property | Mass ( $ m $ ) | [Rotational Inertia](./../rotational-inertia/) ( $ I $ ) |
...
For objects that are undergoing both translational motion (moving through space) and rotational motion (spinning), the total kinetic energy of the system is the sum of both types:
 $$ K_{total} = K_{trans} + K_{rot} $$   $$ K_{total} = \frac{1}{2}mv_{cm}^2 + \frac{1}{2}I_{cm}\omega^2 $$  
Note that for the rotation term, the [Rotational Inertia](./../rotational-inertia/) must be calculated about the center of mass ( $ I_{cm} $ ) if the object is rotating about its center while moving linearly. This concept is central to understanding [Rolling](./../rolling/) motion without slipping.
...
*   **Work-Energy Theorem:** Just as net work changes linear kinetic energy, a net torque acting on a rotating object changes its rotational kinetic energy, a relationship explored in [Torque and Work](./../torque-and-work/).
