...
For a single point charge  $ Q $ , the electric field at a distance  $ r $  can be derived from [Electric Charge and Electric Force](./../electric-charge-and-electric-force/) (Coulomb's Law).
...
When dealing with objects rather than point charges, we treat the charge as distributed over a line, surface, or volume. Calculating the field requires [Calculus-Based Electric Fields](./../calculus-based-electric-fields/), where we integrate over the charge distribution:
...
The electric field is not only related to force but also to potential. It can be viewed as the negative gradient of the [Electric Potential](./../electric-potential/). In one dimension, this is expressed as:
