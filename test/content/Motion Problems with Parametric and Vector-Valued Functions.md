+++
 title = 'Motion Problems with Parametric and Vector-Valued Functions'
[params]
	author = 'Carson West'
+++
...
This topic extends motion analysis from single-variable calculus to objects moving along paths defined parametrically or by vector-valued functions, typically in a 2D plane. It integrates concepts from [Differentiating Parametric Equations](./../differentiating-parametric-equations/) and [Calculus and Vector-Valued Functions](./../calculus-and-vector-valued-functions/).
...
If  $ \mathbf{r}(t) = \langle x(t), y(t) \rangle $ , then the velocity vector is:
 $$  \mathbf{v}(t) = \mathbf{r}'(t) = \langle x'(t), y'(t) \rangle = \left\langle \frac{dx}{dt}, \frac{dy}{dt} \right\rangle  $$  Here,  $ x'(t) $  is the horizontal component of velocity, and  $ y'(t) $  is the vertical component of velocity.
...
The direction of motion at any point is given by the direction of the velocity vector. The slope of the tangent line to the path at time  $ t $  is given by:
 $$  \frac{dy}{dx} = \frac{dy/dt}{dx/dt}  $$  provided  $ dx/dt \neq 0 $ . This is a direct application of [Differentiating Parametric Equations](./../differentiating-parametric-equations/).
...
The total distance a particle travels along its path from time  $ t=a $  to  $ t=b $  is the arc length of the path over that interval. This is calculated by integrating the speed function.
 ...
 $$  \text{Total Distance} = \int_a^b ||\mathbf{v}(t)|| \, dt = \int_a^b \sqrt{\left(\frac{dx}{dt}\right)^2 + \left(\frac{dy}{dt}\right)^2} \, dt  $$  This formula is consistent with the general formula for [Arc Lengths of Parametric Equations](./../arc-lengths-of-parametric-equations/).
...
If the initial position is  $ \mathbf{r}(a) = \langle x(a), y(a) \rangle $  and the final position is  $ \mathbf{r}(b) = \langle x(b), y(b) \rangle $ , then the displacement vector is:
 $$  \Delta \mathbf{r} = \mathbf{r}(b) - \mathbf{r}(a) = \left\langle \int_a^b x'(t) \, dt, \int_a^b y'(t) \, dt \right\rangle  $$  The net change in  $ x $ -position is  $ \int_a^b x'(t) \, dt = x(b) - x(a) $ .
...
If you are given the initial position  $ \mathbf{r}(t_0) = \langle x(t_0), y(t_0) \rangle $  and the velocity vector  $ \mathbf{v}(t) = \langle x'(t), y'(t) \rangle $ , you can find the position at any time  $ t $  using integration:
 ...
 $$  x(t) = x(t_0) + \int_{t_0}^t x'(u) \, du  $$   $$  y(t) = y(t_0) + \int_{t_0}^t y'(u) \, du  $$  Thus,
...
## [When is a Particle Moving Up/Down, Left/Right?](./../when-is-a-particle-moving-up/down,-left/right?/)
...
## [Particle Direction Change](./../particle-direction-change/)
