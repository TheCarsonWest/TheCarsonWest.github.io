...
When a switch is closed to connect an uncharged capacitor ( $ C $ ) in series with a resistor ( $ R $ ) and an EMF source ( $ \mathcal{E} $ ), the capacitor begins to accumulate charge. By applying [Kirchhoff’s Loop Rule](./../kirchhoff’s-loop-rule/), the potential differences sum to zero:
...
Understanding how to analyze these circuits involves applying [Kirchhoff’s Junction Rule](./../kirchhoff’s-junction-rule/) and [Kirchhoff’s Loop Rule](./../kirchhoff’s-loop-rule/) to determine initial and steady-state conditions before solving the differential equations for time-dependent behavior.