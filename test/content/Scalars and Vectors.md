...
*   **Examples:**
    *   **Displacement**: The change in position from an initial point to a final point, including direction (e.g., 5 km East). (See: [Displacement, Velocity, and Acceleration](./../displacement,-velocity,-and-acceleration/))
    ...
    *   **Force**: A push or pull with a specific direction (e.g., 10 N upwards). (See: [Forces and Free-Body Diagrams](./../forces-and-free-body-diagrams/))
    *   **Momentum**: The product of mass and velocity (e.g.,  $ 50 \text{ kg} \cdot \text{m/s} $  West). (See: [Linear Momentum](./../linear-momentum/))
...
*   If  $ \theta $  is the angle the vector makes with the positive x-axis:
     $$ A_x = A \cos \theta $$       $$ A_y = A \sin \theta $$  *   The magnitude of the vector can be found using the Pythagorean theorem:
     $$ A = \sqrt{A_x^2 + A_y^2} $$  *   The direction (angle) of the vector can be found using the inverse tangent function:
     $$ \theta = \arctan\left(\frac{A_y}{A_x}\right) $$      (Care must be taken to ensure the angle is in the correct quadrant based on the signs of  $ A_x $  and  $ A_y $ ).
...
*   **Analytical (Component) Method**: The most common and precise method. To add (or subtract) vectors, add (or subtract) their corresponding components.
    If  $ \vec{A} = \langle A_x, A_y \rangle $  and  $ \vec{B} = \langle B_x, B_y \rangle $ :
     $$ \vec{A} + \vec{B} = \langle A_x + B_x, A_y + B_y \rangle $$       $$ \vec{A} - \vec{B} = \langle A_x - B_x, A_y - B_y \rangle $$  
...
Understanding scalars and vectors is foundational for nearly all topics in AP Physics C, from [Motion in Two or Three Dimensions](./../motion-in-two-or-three-dimensions/) to [Electric Fields](./../electric-fields/) and [Magnetic Fields](./../magnetic-fields/).